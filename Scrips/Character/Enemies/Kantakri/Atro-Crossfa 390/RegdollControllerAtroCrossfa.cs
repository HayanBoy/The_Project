...
public class RegdollControllerAtroCrossfa : MonoBehaviour
{
    [SerializeField] GameObject body, rightLegUp, rightLegDown, leftLegUp, leftLegDown;
    ...
    public void ActiveRagdoll()
    {
        ...
        Transform damageCarrierInfector1 = transform.Find("Body_1"); //자식 오브젝트 위치 찾기
        ...
        //damageCarrierInfector1.GetComponent<BoxCollider2D>().isTrigger = true; //사격에서 제외하도록 트리거 비활성화
        damageCarrierInfector1.GetComponent<BoxCollider2D>().enabled = false; //사격에서 제외하도록 비활성화
        damageCarrierInfector1.GetComponent<CapsuleCollider2D>().enabled = false; //사격에서 제외하도록 콜라이더 해제
    ...
    public void DisableRagdoll()
    {
        ...
        Transform damageCarrierInfector2 = transform.Find("Body_1/Missile launch base1");
        Transform damageCarrierInfector3 = transform.Find("Leg damage");
        damageCarrierInfector1.GetComponent<BoxCollider2D>().enabled = true; //사격에서 제외하도록 비활성화
        damageCarrierInfector1.GetComponent<CapsuleCollider2D>().enabled = true; //사격을 허용하도록 콜라이더 가동
        damageCarrierInfector2.GetComponent<BoxCollider2D>().enabled = true;
        damageCarrierInfector3.GetComponent<BoxCollider2D>().enabled = true;
        damageCarrierInfector3.GetComponent<CapsuleCollider2D>().enabled = true;
        damageCarrierInfector1.gameObject.layer = 13;
        damageCarrierInfector2.gameObject.layer = 13;
        damageCarrierInfector3.gameObject.layer = 13;
