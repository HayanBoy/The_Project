...
public class ShieldExplosionDamage : MonoBehaviour
{
    private Shake shake;
            ...
            if (collision.CompareTag("Shield"))
            {
                if (gameObject.activeSelf == true)
                {
                    if (collision.CompareTag("Shield"))
                    {
                        ...
                        shieldAsoShiioshare.ShieldDamageExplosion(false); //폭발 데미지를 받은 것으로 간주. 이럴 경우 방어능력으로 인해 더 낮은 데미지로 전달됨.
                        StartCoroutine(shieldAsoShiioshare.DamageShieldCharacter(damage * ShieldAddDamage, 0.0f));
