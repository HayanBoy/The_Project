...
public class BuckBullet1 : MonoBehaviour
{
    float AmmoVelocity;
    ...
    void Update()
    {
        ...
        {
            transform.Translate((transform.right * 1 + transform.up * 0.4f) * AmmoVelocity * Time.deltaTime);
        }
        else
        {
            transform.Translate((transform.right * -1 + transform.up * 0.4f) * AmmoVelocity * Time.deltaTime);
    ...
    void FinalExplosion()
    {
        ...
        SoundManager.instance.SFXPlay4("DT-37 Fire Sound", Hydra56AmmoBoom);
    }
}